...
class Serret_Frenet_Guidance:
    ...
    def __init__(self,spline):
        ...
        self.kappa = 0.1
        self.gamma = 3.0
        self.delta = 10  #look ahead distance 
    ...
    def guidance_command(self,stvar):
        ...
        s,e = eps[:]    # s is along track erro and e is crosstrack error 
        

        Xr = atan(-e/self.delta)
        Upp = U*cos(X-Xt)+self.gamma*s
        t_dot = Upp/sqrt(xdd**2+ydd**2) #rate of change of point on curve or theta dot if curve is parametrised by theta 
        
        Xd = Xt + Xr
        Ud = self.kappa*sqrt(self.delta**2+e**2)*10  
        #Ud = 0.5098
        xd = pd[0][0]
        yd = pd[1][0]
...
class Agent():
    ...
    def simulation(self,spline,time,xspl,initial):
        ...
        while i<n:
            ...
            if abs(t-max(xspl)) < 0.5:  # after what precentage of covering the the path it should stop 0.1 means 90% to cover 
